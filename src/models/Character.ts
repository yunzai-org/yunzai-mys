...
const detailKey =
  'title,allegiance,birth,astro,desc,cncv,jpcv,costume,baseAttr,growAttr,materials,talent,talentData,cons,passive,attr,sp'.split(
    ','
  )
...
class Character extends Base {
  // 默认获取的数据
  static _dataKey =
    'id,name,abbr,title,star,elem,allegiance,weapon,birthday,astro,cncv,jpcv,desc,talentCons'

  constructor({ id, name = '', elem = '', game = 'gs' }) {
    super()
    // 检查缓存
    let cacheObj = this._getCache(
      CharId.isTraveler(id)
        ? `character:${id}:${elem || 'anemo'}`
        : `character:${id}`
    )
    if (cacheObj) {
      return cacheObj
  ...
  checkWifeType(type) {
    ...
    let key =
      ['girlfriend', 'boyfriend', 'daughter', 'son'][type] || 'girlfriend'
  ...
  getImgs(costume = '') {
    ...
    if (!this._imgs[cacheId]) {
      if (this.isSr) {
        ...
        this._imgs[cacheId] = CharImg.getImgs(
          this.name,
          costumeIdx,
          this.isTraveler ? this.elem : '',
          this.weaponType,
          this.talentCons
        )
      }
  ...
  getDetail() {
    ...
    try {
      ...
      this.meta = this.meta || {}
      this.meta._detail = Data.readJSON(
        `resources/meta-${this.game}/character/${name}/data.json`,
        'miao'
      )
    } catch (e) {
      console.log(e)
