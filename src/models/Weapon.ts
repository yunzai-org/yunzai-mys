...
class Weapon extends Base {
  ...
  getDetail() {
    ...
    const path = this.isGs
      ? 'resources/meta-gs/weapon'
      : 'resources/meta-sr/weapon'
    try {
      this._detail = Data.readJSON(
        `${path}/${this.type}/${this.name}/data.json`,
        'miao'
      )
    } catch (e) {
      console.log(e)
  ...
  calcAttr(level, promote = -1) {
    ...
    let atkBase =
      valueLeft * 1 +
      ((valueRight - valueLeft) * (level - lvLeft)) / (lvRight - lvLeft)
    ...
    let value =
      valueLeft + (stepCount - Math.ceil((lvRight - level) / 5)) * valueStep
    return {
      atkBase,
      attr: {
        key: wAttr.bonusKey,
