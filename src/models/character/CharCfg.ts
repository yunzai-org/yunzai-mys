...
let cfgMap = {
  ...
  exists(char, file) {
    return fs.existsSync(
      `${miaoPath}/resources/meta-${this.game}/character/${char}/${file}.js`
    )
  },
  async getCfg(char, file, module = '') {
    let cfg = await Data.importModule(
      `resources/meta-${this.game}/character/${char}/${file}.js`,
      'miao'
    )
    if (module) {
      return cfg[module]
...
let CharCfg = {
  ...
  getCalcRule(char) {
    let cfg =
      cfgMap.char[char.isTraveler ? `旅行者/${char.elem}` : char.name]?.calc
